...
function audit(type, event, user = 'GMM: A.Okonkwo') {
  DB.audit.unshift({ id: 'evt_' + nanoid(8), type, event, user, ts: new Date().toISOString() });
}

// ── Health & config ───────────────────────────────────────────────────────
app.get('/api/health', (_req, res) => res.json({ status: 'ok', messaging: messagingStatus(), time: new Date().toISOString() }));
app.get('/api/config', (_req, res) => res.json({ messaging: messagingStatus(), institution: DB.institutions.find(i => i.id === DB.activeInstitution) }));
...
app.listen(PORT, () => console.log(`\n🏦 IGE Banking API running on http://localhost:${PORT}\n   Email: ${messagingStatus().email.provider}  ·  SMS: ${messagingStatus().sms.provider}\n`));
